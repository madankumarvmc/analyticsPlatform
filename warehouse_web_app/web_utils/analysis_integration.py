...
class WebAnalysisIntegrator:
    ...
    def _run_enhanced_analysis(self, enriched_data: pd.DataFrame, parameters: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            self.logger.info(f"🔍 ANALYSIS PARAMETERS DEBUG:")
            self.logger.info(f"  - enable_advanced: {enable_advanced}")
            self.logger.info(f"  - generate_charts: {generate_charts}")
            self.logger.info(f"  - generate_reports: {generate_reports}")
            self.logger.info(f"  - Full parameters: {list(parameters.keys())}")
            ...
            if generate_charts:
                self.logger.info("Starting chart generation for enhanced analysis")
                
                # Generate basic charts first
                try:
                    if hasattr(pipeline, 'chart_generator') and pipeline.chart_generator:
                        self.logger.info("Generating basic charts")
                        pipeline.chart_generator.generate_all_charts(combined_results)
                        self.logger.info("✅ Basic charts generated successfully")
                except Exception as e:
                    self.logger.error(f"❌ Basic chart generation failed: {e}")
                
                # Generate advanced charts
                try:
                    # 🚨 DEBUG: Check advanced chart generator availability
                    self.logger.info(f"🔍 CHART GENERATOR DEBUG:")
                    self.logger.info(f"  - hasattr advanced_chart_generator: {hasattr(pipeline, 'advanced_chart_generator')}")
                    if hasattr(pipeline, 'advanced_chart_generator'):
                        self.logger.info(f"  - advanced_chart_generator is not None: {pipeline.advanced_chart_generator is not None}")
                        self.logger.info(f"  - advanced_chart_generator type: {type(pipeline.advanced_chart_generator)}")
                    
                    if hasattr(pipeline, 'generate_advanced_charts') and pipeline.advanced_chart_generator is not None:
                        self.logger.info("✅ Starting advanced chart generation")
                        chart_results = pipeline.generate_advanced_charts(basic_results, enhanced_results)
                        if chart_results:
                            self.logger.info(f"✅ Successfully generated {len(chart_results)} advanced charts:")
                            for chart_name, chart_path in chart_results.items():
                                self.logger.info(f"  - {chart_name}: {chart_path}")
                            combined_results['chart_paths'] = chart_results
                        else:
                            self.logger.warning("❌ No advanced charts were generated by the pipeline")
                    else:
                        self.logger.warning("❌ Advanced chart generator not available")
                        self.logger.warning("  This means the EnhancedWarehouseAnalysisPipeline was initialized with generate_advanced_charts=False")
                except Exception as e:
                    self.logger.error(f"❌ Advanced chart generation failed: {e}")
                    import traceback
                    self.logger.error(f"Advanced chart generation traceback: {traceback.format_exc()}")
            
            # Note: LLM summaries will be generated during Word report generation if needed
            
            # Generate Word report if enabled (AFTER charts and summaries)  
            if generate_reports:
                try:
                    if hasattr(pipeline, 'word_generator') and pipeline.word_generator:
                        self.logger.info("Generating Word report via pipeline")
                        
                        # Generate LLM summaries first if not already done
                        if hasattr(pipeline, 'llm_integration') and pipeline.llm_integration and 'llm_summaries' not in combined_results:
                            self.logger.info("Generating LLM summaries for Word report")
                            llm_summaries = pipeline.llm_integration.generate_all_summaries(combined_results)
                            combined_results['llm_summaries'] = llm_summaries
                        
                        # Generate Word report with all components
                        chart_paths = combined_results.get('chart_paths', {})
                        llm_summaries = combined_results.get('llm_summaries', {})
                        
                        self.logger.info(f"Word report inputs - Charts: {len(chart_paths)}, Summaries: {len(llm_summaries)}")
                        
                        word_path = pipeline.word_generator.generate_word_report(
                            combined_results, chart_paths, llm_summaries
                        )
                        if word_path:
                            combined_results['word_report_path'] = word_path
                            self.logger.info(f"✅ Word report generated: {word_path}")
                        else:
                            self.logger.warning("Word report generation returned empty path")
                    else:
                        self.logger.warning("Word report generator not available")
                except Exception as e:
                    self.logger.error(f"❌ Word report generation failed: {e}")
                    import traceback
                    self.logger.error(f"Word report generation traceback: {traceback.format_exc()}")
